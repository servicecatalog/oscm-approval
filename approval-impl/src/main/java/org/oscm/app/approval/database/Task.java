...
public class Task {
    ...
    public static enum ApprovalStatus {
        WAITING_FOR_APPROVAL(1), TIMEOUT(2), FAILED(3), APPROVED(4), REJECTED(
                5), NOTIFICATION(6), WAITING_FOR_CLEARANCE(
                        7), CLEARANCE_GRANTED(8);
    ...
    public Map<String, String> getTriggerProcessData() {
        ...
        TriggerProcessData processData = JSONMapper
                .toTriggerProcessData(description);
        ...
        props.put("user.additional_name",
                processData.ctmg_user.additional_name);
        props.put("user.salutation",
                processData.ctmg_user.salutation);
        ...
        if (processData.ctmg_service != null) {
            ...
            props.put("service.technicalId",
                    processData.ctmg_service.technicalId);
            props.put("service.name", processData.ctmg_service.name);
            props.put("service.seller.id", processData.ctmg_service.seller.id);
            props.put("service.seller.key",
                    processData.ctmg_service.seller.key);
            props.put("service.seller.name",
                    processData.ctmg_service.seller.name);

            props.put("service.price.freePeriod",
                    processData.ctmg_service.price.freePeriod);
            props.put("service.price.oneTimeFee",
                    processData.ctmg_service.price.oneTimeFee);
            props.put("service.price.pricePerPeriod",
                    processData.ctmg_service.price.pricePerPeriod);
            props.put("service.price.pricePerUser",
                    processData.ctmg_service.price.pricePerUser);
            props.put("service.price.type",
                    processData.ctmg_service.price.type);

            for (String key : processData.ctmg_service.params.keySet()) {
                props.put("service.params." + key + ".value",
                        processData.ctmg_service.params.get(key).value);
                props.put("service.params." + key + ".label",
                        processData.ctmg_service.params.get(key).label);
                props.put("service.params." + key + ".id",
                        processData.ctmg_service.params.get(key).id);
            }
        }
        ...
        props.put("organization.address",
                processData.ctmg_organization.address);
        ...
        try {
            ...
            if ("GrantClearance".equals(processData.ctmg_trigger_id)) {
                ...
                cred = das.loadOrgAdminCredentials(
                        processData.ctmg_organization.id);
            }
            props.put("admin.userid", cred.getUserId());
            props.put("admin.password", cred.getPassword());

        } catch (Exception e) {
            logger.error("Failed to retrieve org admin login for organization "
                    + processData.ctmg_organization.id, e);
        }
