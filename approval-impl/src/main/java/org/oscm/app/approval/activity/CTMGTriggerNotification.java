...
public class CTMGTriggerNotification extends Activity {
        ...
        return new WebServiceTask<T>(serv) {
            ...
            public Object execute(T svc) throws ProcessException {
                try {
                    ...
                    if (approve) {
                        logger.debug("approve action with orgid: " + orgid + " triggerId: "
                                + triggerkey);
                        trigSvc.approveAction(Long.parseLong(triggerkey));
                    } else {
                        logger.debug(
                                "reject action with orgid: " + orgid + " triggerId: " + triggerkey);
    ...
    private void notifyCTMGTrigger(String triggerkey, String orgid, String reason, boolean approve)
            throws ProcessException {
        logger.debug("triggerkey: " + triggerkey + "orgid: " + orgid + " approve: " + approve);

        try {
            BesClient.runWebServiceAsOrganizationAdmin(orgid,
                    createTriggerTask(TriggerService.class, approve));
